...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO stg.PAFLOAD_WithHash (
            ...
            [RowHash], [LocalityKey], [ThoroughfareKey], [DepThoroughfareDescriptorKey], -- Corrected Column Name
        ...
        SELECT
            ...
            HASHBYTES('SHA2_256', ISNULL(TRIM(DepThoroughfareDescriptor), 'NV')), -- Correctly calculated key
        ...
        MERGE dbo.tblRMThoroughfareDescriptor AS T USING (SELECT DISTINCT DepThoroughfareDescriptorKey, DepThoroughfareDescriptor FROM stg.PAFLOAD_WithHash WHERE DepThoroughfareDescriptor IS NOT NULL) AS S ON (T.ThoroughfareKey=S.DepThoroughfareDescriptorKey) WHEN NOT MATCHED BY TARGET THEN INSERT (ThoroughfareKey,Descriptor,Username) VALUES (S.DepThoroughfareDescriptorKey,S.DepThoroughfareDescriptor,SUSER_SNAME()) WHEN NOT MATCHED BY SOURCE THEN DELETE;
        ...
        MERGE dbo.tblRMSubBuildingName AS T USING (SELECT DISTINCT SubBuildingNameKey, SubBuildingName FROM stg.PAFLOAD_WithHash WHERE SubBuildingName IS NOT NULL) AS S ON (T.SubBuildingKey=S.SubBuildingNameKey) WHEN NOT MATCHED BY TARGET THEN INSERT (SubBuildingKey,Descriptor,Username) VALUES (S.SubBuildingNameKey,S.SubBuildingName,SUSER_SNAME()) WHEN NOT MATCHED BY SOURCE THEN DELETE;
        ...
        SELECT HASHBYTES('SHA2_256', UDPRN), Postcode, LocalityKey, ThoroughfareKey, ThoroughfareKey, DepThoroughfareDescriptorKey, BuildingNameKey, SubBuildingNameKey, CompanyKey, PostCodeType, DelPointSuffix, POBox, Outcode, Incode, SUSER_SNAME() FROM dbo.PAFLOAD;
